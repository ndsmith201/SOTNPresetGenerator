...
export function JsonPreview({ preview, onCopy, community }: { community?: boolean; preview: JsonObject | null; onCopy: () => void }) {
  const json = preview ? JSON.stringify(preview, null, 2) : "";
  return (
    <aside className="preview-pane" aria-labelledby="preview-title">
      <div className="preview-heading"><div><span className="step-label">02 / Preview</span><h2 id="preview-title">Preset JSON</h2><p>{community ? "Original community configuration." : "Live preview of your configuration."}</p></div><button className="icon-button" type="button" aria-label="Copy JSON" title="Copy JSON" onClick={onCopy}><Icon name="copy" /></button></div>
      ...
      <div className="preview-footer"><span className="draft-status"><span aria-hidden="true" /> {community ? "Community preset" : "Local draft"}</span></div>
    </aside>
  );
}
