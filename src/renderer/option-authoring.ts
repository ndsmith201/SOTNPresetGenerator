...
export type WritePlacement = "default" | "game-init" | "item-init" | "after-relics" | "main-block";
...
export function optionDraft(option?: DatabaseOption | null): OptionDraft {
  return {
    ...
    placement: option?.mainBlock ? "main-block" : option?.itemInit ? "item-init" : option?.gameInit ? "game-init" : option?.statEdit ? "after-relics" : "default",
...
export function draftInput(draft: OptionDraft): CreateOptionInput {
  ...
  return {
    ...
    ...(draft.placement === "main-block" ? { mainBlock: true } : draft.placement === "item-init" ? { itemInit: true } : draft.placement === "game-init" ? { gameInit: true } : draft.placement === "after-relics" ? { statEdit: true } : {})
  };
}
