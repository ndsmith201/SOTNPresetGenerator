...
- **Add to my presets** saves a local copy with the original authors and returns to the preset library. **Use as template** creates a copy and opens it in the editor. Your name is added only when you change its name, description, options, or settings, and only if it is not already listed (ignoring case and surrounding spaces). Reverting all edits preserves the original authors. The editor rebuilds location rules and does not resolve inherited presets, so review the resulting JSON before exporting. **Copy JSON** in the community view copies the original submission.
...
- Share your current preset using **Share** in the top right. Option cards have an actions menu: View for bundled options, and Edit, Share, and Delete for local options. Deleting a local option asks for confirmation and removes it from saved preset selections. Both open a confirmation dialog with optional JSON review; canceling publishes nothing. Presets are exported and built with their current changes before submission when needed. Sharing a preset with an existing name updates its JSON when your signed-in username appears in the existing preset's `metadata.author` list, preserving its catalog ID and votes. Names and usernames match regardless of case or surrounding spaces. A collision without authorship is rejected; choose a different preset name. This behavior requires the API's preset-update support to be deployed. Options and presets with new names create new entries. Option IDs and read-only flags are excluded, and uncertain failures are not retried automatically.
...
The **Share preset** dialog includes a required description, prefilled from the current preset. Edits appear in the JSON review and are saved to the local draft when you confirm sharing. The description is included in the exported and shared preset's `metadata.description`; changing it triggers a rebuild before submission. Canceling the dialog discards unconfirmed description edits.
...
Community cards display preset metadata authors or the creator's username. The API resolves Cognito creator IDs to `createdByUsername`; this requires the matching SOTNPresetAPI deployment. Options save that username on creation and reuse it on reads. Older options need the API project's `scripts/backfill-option-authors.ps1` backfill. An unresolved Cognito ID displays as "Unknown author".
...
All bundled options are read-only: use the eye button to inspect and select/copy their values. Their settings cannot be saved over from this dialog. Startup also protects previously installed bundled options that were editable, preserving their IDs and preset selections. Newly created custom options remain editable.
...
| Preset author | Adds your name only when a preset differs from its source, without duplicating an existing author. Uses the signed-in username, or the name saved here when signed out; leave it blank to keep only the original authors. The saved setting is preserved when signing in or out. |
...
| `npm run release <major\|minor\|patch>` | Bump the version and push the version commit and tag to start a release. |
...
Run `npm run options:dump` explicitly to export your current `%APPDATA%/sotn-preset-generator/options.sqlite` to `database/options-dump.sql`. Set `SOTN_OPTIONS_DATABASE` to use another database. Export opens the source read-only and includes committed changes even while the app is open. Review and commit the updated snapshot when changing the bundled catalog. Packaging and version bumps do not refresh it or require a local options database. Packaging validates the saved snapshot and fails if it is missing or invalid. Every installer and ZIP bundles the snapshot, and `options-dump.sql` is also uploaded as a separate release asset.

On first launch after installation or an update, the app compares the bundled snapshot's SHA-256 fingerprint with the last applied snapshot. A changed snapshot deletes and restores only tracked bundled options, including their descriptions and write data. This restores deleted bundled entries and removes entries retired from the bundle. All tracked bundled options are made read-only on every launch, including options from older snapshots that marked them editable. An unchanged snapshot otherwise preserves the catalog on later launches. Custom options, presets, settings, and export records remain in place.

The `bundled_options` table maps snapshot IDs to stable local option IDs so saved preset selections continue to refer to the same options. If a new bundled ID is already used by a custom option, the bundled entry receives a separate local ID. Mappings for retired entries are retained for future restoration. When upgrading from a version without tracking, registered read-only entries and exact copies of the current snapshot are adopted as bundled. The immutable `database/bundled-options-baseline.json` also identifies previously shipped editable entries by ID and creation timestamp, allowing their replacement or retirement without confusing custom entries that reuse those IDs. Keep this baseline unchanged when exporting new snapshots. Unrecognized editable entries are preserved as local options. Very old catalogs without registration flags adopt matching ID/name pairs. The replacement, mapping updates, fingerprint, and pending SQL data migrations run in one transaction, so failures roll back and retry on the next launch.

`001-built-in-option-descriptions.sql` supplies the 16 legacy reviewed descriptions once, recording completion in `options_migrations`. Keep previously released migrations unchanged. Ship bundled catalog changes by explicitly exporting and committing a fresh snapshot. For other data migrations, add a new migration to the list in `src/options-database.ts` and the runtime file allowlist in `forge.config.cjs`. Snapshot export includes only the options table, excluding local bundle tracking and migration history.
...
Use `major`, `minor`, or `patch` for the desired version bump; `npm run release -- patch` also works. To include local catalog changes, first run `npm run options:dump`, review the snapshot, and commit it with the release changes. The release command runs `npm version`, then pushes the current branch and the new `v<version>` tag to `origin` atomically, triggering the release workflow. Start with a clean working tree, as required by `npm version`. If the push fails, the commit and tag remain local; use the retry command printed by the script instead of bumping the version again.

To release the current version without a version bump, commit the release changes, including any intentionally refreshed snapshot, then create and push an unused `v<version>` tag matching `package.json`. You can also run **Release Windows** manually in GitHub Actions and enter an existing tag. GitHub-hosted runners validate and package the snapshot committed at that tag. To include later database changes, export and commit a fresh snapshot for a new release.
