...
On first launch after installation or an update, the app compares the bundled snapshot's SHA-256 fingerprint with the last applied snapshot. A changed snapshot deletes and restores only tracked bundled options, including their descriptions, write data, and read-only flags. This restores deleted bundled entries and removes entries retired from the bundle. An unchanged snapshot leaves the catalog alone on later launches. Custom options, presets, settings, and export records remain in place.
